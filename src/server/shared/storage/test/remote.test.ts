...
describe("Test remote storage", () => {
  let remoteStore!: BiomesStorage.Store;
  let rpcServer!: ZrpcServer;
  let store!: BiomesStorage.Store;

  beforeEach(async () => {
    remoteStore = createInMemoryStorage();

    rpcServer = new ZrpcServer();
    rpcServer.install(
      zRemoteStorageService,
      new ExposeStorageService(remoteStore)
    );
    const port = await rpcServer.start();
    store = new RemoteStorage(
      makeClient(zRemoteStorageService, `127.0.0.1:${port}`)
    );
  });
