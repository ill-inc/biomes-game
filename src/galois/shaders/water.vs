...
void main() {
  ...
  _blockCoord = blockCoord(position, uint(direction), texCoord);
  _texCoord = texCoords(position, uint(direction), texCoord);

  // Emit the view-dependent vertex attributes.
  _normal = normalize(normalMatrix * normals[uint(direction)]);
